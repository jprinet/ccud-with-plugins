 ...
 */
public class GradleEnterprisePluginQuarkus implements GradleEnterprisePluginApi {

    private static final Logger LOGGER = LoggerFactory.getLogger(GradleEnterprisePluginQuarkus.class);
...
//    private static final class QuarkusExtensionConfiguration {
        ...
        private void initQuarkusExtensionConfiguration(MojoMetadataProvider.Context context) {
...
//    }
        ...
        buildCache.registerMojoMetadataProvider(context -> {
            context.withPlugin("quarkus-maven-plugin", () -> {
                if ("build".equals(context.getMojoExecution().getGoal())) {
                    ...
                    initQuarkusExtensionConfiguration(context);
    ...
    private void configureQuarkusBuildGoal(MojoMetadataProvider.Context context) {
        ...
        Properties quarkusBuildProperties = loadProperties(context, getDumpConfigFileName());

        // Check required configuration
        if (isQuarkusBuildCacheable(context, quarkusBuildProperties)) {
    ...
    private boolean isQuarkusBuildCacheable(MojoMetadataProvider.Context context, Properties quarkusBuildProperties) {
        return isQuarkusPropertiesUnchanged(context, quarkusBuildProperties)
            && isInContainerBuild(quarkusBuildProperties)
            && isPackagingTypeSupported(quarkusBuildProperties);
    }

    private boolean isQuarkusPropertiesUnchanged(MojoMetadataProvider.Context context, Properties quarkusProperties) {
        Properties quarkusCurrentProperties = loadProperties(context, getCurrentConfigFileName());
